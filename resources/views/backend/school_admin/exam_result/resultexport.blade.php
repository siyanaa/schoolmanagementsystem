    ...
    <thead>
        <tr>
            ...
            @foreach ($examinations->subjectByRoutine as $routine)
                <th colspan="6">{{ $routine->subject }}</th>
            @endforeach
            <th>GPA</th>
        </tr>
        <tr>
        ...
        @foreach ($studentSessions as $studentSession)
            <tr>
                ...
                @foreach ($examinations->subjectByRoutine as $routine)
                    ...
                    <td>{{ $resultData['examResult'] ? $resultData['examResult']->practical_assessment : '-' }}
                    </td>
                    <td>{{ $resultData['examResult'] ? $resultData['examResult']->theory_assessment : '-' }}
                    </td>
                    <td>{{ $resultData['examResult'] ? $resultData['examResult']->partial_sum : '-' }}
                    </td>
